...
--
-- Locks candidate messages with FOR UPDATE SKIP LOCKED so that concurrent
-- pollers claim disjoint sets of messages instead of blocking on each
-- other's row locks. The locking SELECT lives in a MATERIALIZED CTE so it is
-- executed exactly once: a locking subquery in the UPDATE's FROM clause
-- cannot be materialized and could be re-executed (locking and returning
-- extra rows) while joining. The `attempt_at <= NOW()` re-check on the outer
-- UPDATE is kept as defence in depth.
...
BEGIN
    RETURN QUERY
    WITH messages_to_update AS MATERIALIZED (
        SELECT
            msgs.id
